...
export function Sidebar() {
  ...
  const { sidebarOpen, openQuickTask, closeSidebar } = useUIStore();
    ...
    <>
      ...
      {sidebarOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative flex h-full w-[82vw] max-w-xs flex-col border-r border-border bg-card shadow-2xl">
            <div className="flex items-center justify-between border-b border-border px-4 py-5">
              ...
              <button
                type="button"
                onClick={closeSidebar}
                className="flex h-9 w-9 items-center justify-center rounded-xl bg-accent/60 text-muted-foreground"
                aria-label="Fechar menu"
              >
                <X className="h-5 w-5" />
              </button>
